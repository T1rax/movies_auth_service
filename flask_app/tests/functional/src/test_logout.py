...
async def test_logout(test_config, payload, expected_answer, default_headers, aiohttp_session, db_client, mds_client):
    ...
    response_2 = await aiohttp_session.post(test_config.service_url+'/auth/logout', cookies=cookies_dict_1, headers=default_headers)
    cookies_dict_2 = response_2.cookies

    response_3 = await aiohttp_session.post(test_config.service_url+'/auth/authorize', cookies=cookies_dict_2, headers=default_headers)
