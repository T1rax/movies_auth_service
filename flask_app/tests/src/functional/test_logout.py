...
@pytest.mark.asyncio
async def test_logout(test_config, payload, expected_answer, default_headers, aiohttp_session, db_client, mds_client):

    #Логинимся для получения токенов
    response_1 = await aiohttp_session.post(test_config.service_url+'/auth/v1/sign-in', json=payload, headers=default_headers)
    cookies_dict_1 = response_1.cookies

    response_2 = await aiohttp_session.post(test_config.service_url+'/auth/v1/logout', cookies=cookies_dict_1, headers=default_headers)
    cookies_dict_2 = response_2.cookies

    response_3 = await aiohttp_session.post(test_config.service_url+'/auth/v1/authorize', cookies=cookies_dict_2, headers=default_headers)
